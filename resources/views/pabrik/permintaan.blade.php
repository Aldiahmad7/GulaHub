    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="mb-8 bg-white rounded-xl shadow-sm p-6 border border-gray-100 transition-all duration-300 hover:shadow-md">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    <h2 class="text-2xl font-bold text-gray-800">Permintaan dari Petani</h2>
                    <p class="text-sm text-gray-500 mt-1">Daftar permintaan jadwal panen dari petani</p>
                </div>
            </div>
        </div>
                ...
                <div class="bg-gray-50 px-6 py-4 border-b border-gray-200">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                <h3 class="text-lg font-semibold text-gray-800">Rencana Panen</h3>
                            ...
                            <div class="bg-gray-50 rounded-lg p-5 border border-gray-200 transition-all duration-300 hover:shadow-sm">
                                <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
                                    <div class="flex items-center space-x-4">
                                        ...
                                        <div>
                                            <h4 class="text-gray-900 font-medium">{{ $petani->name }}</h4>
                                            <div class="flex items-center space-x-2 mt-1">
                                                <span class="text-xs text-gray-500">Status:</span>
                                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                                    @if($petani->pivot->status == 'Disetujui')
                                                        bg-green-100 text-green-800
                                                    @elseif($petani->pivot->status == 'Ditolak')
                                    ...
                                    <div class="flex gap-2">
                                        <form method="POST" action="{{ route('pabrik.konfirmasi', [$rencana->id, $petani->id]) }}">
                                            @csrf
                                        ...
                                        <button type="button"
                                            onclick="openTolakModal({{ $rencana->id }}, {{ $petani->id }})"
        ...
        @empty
            <div class="text-center py-12 opacity-0 animate-fadeIn">
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-8 max-w-md mx-auto transition-all duration-300 hover:shadow-md">
                    ...
                    <p class="text-gray-500 text-sm transition-all duration-300">Belum ada permintaan dari petani saat ini.</p>
                </div>
            </div>
        @endforelse
    </div>
...
<script>
function openTolakModal(rencanaId, petaniId) {
    const form = document.getElementById('formTolak');
    form.action = `/pabrik/konfirmasi/${rencanaId}/${petaniId}`;
