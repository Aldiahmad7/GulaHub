...
<div class="container mx-auto px-4 pt-20 max-w-3xl">
    <h3 class="text-2xl font-semibold mb-8 text-center text-gray-800">Permintaan Petani</h3>
            ...
            <div class="p-5 space-y-6">
                @foreach ($rencana->pengaju as $petani)
                    <div class="bg-white rounded-lg border border-gray-200 p-4 shadow-sm flex flex-col sm:flex-row sm:justify-between sm:items-center">
                        <div>
                            <p class="text-gray-900 font-semibold">{{ $petani->name }}</p>
                            <p class="text-sm text-gray-500 mt-1">Status: <em>{{ $petani->pivot->status }}</em></p>
                        </div>
                        <form method="POST" action="{{ route('pabrik.konfirmasi', [$rencana->id, $petani->id]) }}" class="mt-4 sm:mt-0">
                            @csrf
