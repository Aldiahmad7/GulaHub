...
@section('content')
<div class="container">
    <h3>Rencana Giling: {{ $tahunDipilih }}</h3>

    <!-- Filter Tahun -->
    <form method="GET" action="{{ route('pabrik.rencanagiling') }}">
                            ...
                            <div class="modal fade" id="modalEdit{{ $item->id }}" tabindex="-1">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <!-- Form Edit -->
                                        <form method="POST" action="{{ route('rencanagiling.update', $item->id) }}">
                                            @csrf
                                            @method('PUT')
                                            <div class="modal-header">
                                                <h5 class="modal-title">Edit Data Giling</h5>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                                            </div>
                                            <div class="modal-body">
                                                <div class="mb-3">
                                                    <label>Kebutuhan Giling</label>
                                                    <input type="text" class="form-control" name="kebutuhan_giling" value="{{ $item->kebutuhan_giling }}" required>
                                                </div>
                                                <div class="mb-3">
                                                    <label>Tanggal</label>
                                        ...
                                        <!-- Form Hapus -->
                                        <form method="POST" action="{{ route('rencanagiling.destroy', $item->id) }}" onsubmit="return confirm('Yakin hapus data ini?')">
                                            @csrf
                                            @method('DELETE')
...
<div class="modal fade" id="modalTambah" tabindex="-1">
    <div class="modal-dialog">
        <form method="POST" action="{{ route('rencanagiling.store') }}">
            @csrf
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Tambah Rencana Giling</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <div class="mb-3">
                        <label>Kebutuhan Giling</label>
                        <input type="text" class="form-control" name="kebutuhan_giling" required>
                    </div>
                    <div class="mb-3">
                        <label>Tanggal</label>
