...
<div class="container mx-auto px-4 pt-20">
    <h3 class="text-xl font-semibold mb-4">Rencana Giling - {{ $pabrik->name }} ({{ $tahunDipilih }})</h3>

    <!-- Filter Tahun -->
    <form method="GET" action="{{ route('petani.rencanagiling', $pabrik->id) }}" class="mb-4">
            ...
            @if (isset($dataPerBulan[$bulan]) && count($dataPerBulan[$bulan]) > 0)
                ...
                <dialog id="modalDetail{{ $item->id }}" class="rounded-lg p-6 w-full max-w-md backdrop:bg-black/50">
                    <div class="bg-white rounded shadow p-4">
                        <h3 class="text-lg font-semibold mb-4">Detail Rencana Giling</h3>
                        <p><strong>Kebutuhan Giling:</strong> {{ $item->kebutuhan_giling }}</p>
                        <p><strong>Tanggal:</strong> {{ \Carbon\Carbon::parse($item->tanggal)->translatedFormat('d F Y') }}</p>

                        <form method="POST" action="{{ route('petani.ajukan', $item->id) }}" class="mt-6 text-center">
                            @csrf
                            <input type="hidden" name="tanggal" value="{{ $item->tanggal }}">
                            <input type="hidden" name="pabrik_id" value="{{ $pabrik->id }}">
                            <button type="submit" class="px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded">Ajukan ke Pabrik</button>
                        </form>
