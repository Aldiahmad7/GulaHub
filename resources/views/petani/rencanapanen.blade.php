...
<div class="container mx-auto px-4 pt-20">
    <h3 class="text-xl font-semibold mb-4">Rencana Panen: {{ $tahunDipilih }}</h3>

    <!-- Filter Tahun -->
    <form method="GET" action="{{ route('petani.rencanapanen') }}" class="mb-4">
            ...
            @if (isset($dataPerBulan[$bulan]) && count($dataPerBulan[$bulan]) > 0)
                ...
                <dialog id="modalEdit{{ $item->id }}" class="rounded-lg w-full max-w-md">
                    <form method="POST" action="{{ route('petani.rencanapanenupdate', $item->id) }}" class="space-y-4">
                        @csrf
                        @method('PUT')
                        <h3 class="text-lg font-semibold">Edit Data Panen</h3>
                        <div>
                            <label class="block text-sm font-medium">Jenis Tebu</label>
                            <input type="text" name="jenis_tebu" value="{{ $item->jenis_tebu }}" class="w-full border rounded px-3 py-2">
                        </div>
                        <div>
                            <label class="block text-sm font-medium">Total Panen</label>
                            <input type="text" name="total_panen" value="{{ $item->total_panen }}" class="w-full border rounded px-3 py-2">
                        </div>
                        <div>
                    ...
                    <form method="POST" action="{{ route('petani.rencanapanendestroy', $item->id) }}" onsubmit="return confirm('Yakin hapus data ini?')">
                        @csrf
                        @method('DELETE')
...
<dialog id="modalTambah" class="rounded-lg w-full max-w-md">
    <form method="POST" action="{{ route('petani.rencanapanenstore') }}" class="space-y-4">
        @csrf
        <h3 class="text-lg font-semibold">Tambah Rencana Panen</h3>
        <div>
            <label class="block text-sm font-medium">Jenis Tebu</label>
            <input type="text" name="jenis_tebu" required class="w-full border rounded px-3 py-2">
        </div>
        <div>
            <label class="block text-sm font-medium">Total Panen</label>
            <input type="text" name="total_panen" required class="w-full border rounded px-3 py-2">
        </div>
        <div>
            <label class="block text-sm font-medium">Tanggal</label>
