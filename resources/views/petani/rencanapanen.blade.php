    ...
    <div class="max-w-7xl mx-auto mt-12 px-4 sm:px-6 lg:px-8">
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-8">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h1 class="text-2xl font-bold text-gray-900">Rencana Panen</h1>
                        <p class="text-gray-600 mt-1">Kelola jadwal dan rencana panen untuk tahun {{ $tahunDipilih }}</p>
                    </div>
                </div>

                <div class="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                    <form method="GET" action="{{ route('petani.rencanapanen') }}" class="flex items-center gap-2 bg-gray-50 p-2 rounded-lg">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200 bg-gray-50">
                <h2 class="text-lg font-semibold text-gray-900">Daftar Rencana Panen</h2>
            </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Jenis Tebu</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Total Panen (TON)</th>
                                        ...
                                        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                                            <div class="sm:flex sm:items-start">
                                                <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                                    <h3 class="text-lg leading-6 font-medium text-gray-900 mb-4">Edit Rencana Panen</h3>

                                                    <form method="POST" action="{{ route('petani.rencanapanenupdate', $item->id) }}" class="space-y-4">
                                                        @csrf
                                                        @method('PUT')
                                                        <div>
                                                            <label for="jenis_tebu" class="block text-sm font-medium text-gray-700">Jenis Tebu</label>
                                                            <input type="text" name="jenis_tebu" id="jenis_tebu" value="{{ $item->jenis_tebu }}"
                                                                   class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                                                        </div>

                                                        <div>
                                                            <label for="total_panen" class="block text-sm font-medium text-gray-700">Total Panen (TON)</label>
                                                            <input type="text" name="total_panen" id="total_panen" value="{{ $item->total_panen }}"
                                                                   class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                                                        </div>
            ...
            @else
                <div class="px-6 py-12 text-center">
                    ...
                    <h3 class="text-lg font-medium text-gray-900">Belum ada rencana panen</h3>
                    <p class="mt-1 text-sm text-gray-500">Mulai dengan menambahkan rencana panen pertama Anda</p>
                    <div class="mt-6">
                        <button onclick="openModal('modalTambah')"
                                class="inline-flex items-center px-4 py-2.5 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white text-sm font-medium rounded-lg transition-all shadow-sm hover:shadow-md">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div class="sm:flex sm:items-start">
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                        <h3 class="text-lg leading-6 font-medium text-gray-900 mb-4">Tambah Rencana Panen</h3>

                        <form method="POST" action="{{ route('petani.rencanapanenstore') }}" class="space-y-4">
                            @csrf
                            <div>
                                <label for="jenis_tebu" class="block text-sm font-medium text-gray-700">Jenis Tebu</label>
                                <input type="text" name="jenis_tebu" id="jenis_tebu" required
                                       class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                            ...
                            <div>
                                <label for="total_panen" class="block text-sm font-medium text-gray-700">Total Panen (TON)</label>
                                <input type="text" name="total_panen" id="total_panen" required
                                       class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
